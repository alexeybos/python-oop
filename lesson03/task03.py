...
class Garden(Territory):
    def __init__(self, height, width, owner, tree_count):
        super().__init__(height, width, owner)
        self.tree_count = tree_count

    def plant_tree(self, count):
        self.tree_count += count

    def cut_down_tree(self, count):
        self.tree_count -= count
...
class Field(Territory):
    ...
    def seed(self, seed_culture, productivity, ripening_date):
        self.seed_culture = seed_culture
...
class Plant:
    def __init__(self, name, plant_type, growth_per_day):
        self.name = name
    ...
    def get_info(self):
        return f"{self.name} ({self.plant_type}), высота: {self.height} см"

class Tree(Plant):
    def __init__(self, name, plant_type, growth_per_day, circumference_per_day):
        super().__init__(name, plant_type, growth_per_day)
...
class Potato(Plant):
    def __init__(self, name, plant_type, growth_per_day, tubers_per_height):
        super().__init__(name, plant_type, growth_per_day)
